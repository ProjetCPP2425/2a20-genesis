...
void MainWindow::onvaliderClicked()
{
    ...
    // Insertion
    QSqlQuery query;
    query.prepare("INSERT INTO EMPLOYES (NOM_EM, POSTE, NUMTEL, EMAIL, PASSWORD, GENRE, DN, QUESTION, REPONSE) "
...
void MainWindow::val()
{
    QSqlQuery query("SELECT NOM_EM, POSTE, NUMTEL, EMAIL, PASSWORD,GENRE , DN FROM EMPLOYES");
    ...
    while (query.next()) {
        ...
        ui->tw->setItem(row, 0, new QTableWidgetItem(query.value("NOM_EM").toString()));
...
void MainWindow::ondelClicked()
{
    ...
    for (QTableWidgetItem *item : selectedItems) {
        ...
        QSqlQuery query;
        query.prepare("DELETE FROM EMPLOYES WHERE NOM_EM = :nom");
        query.bindValue(":nom", nom);
...
void MainWindow::onupClicked()
{
    ...
    QSqlQuery query;
    query.prepare("UPDATE EMPLOYES SET NOM_EM = :nom, POSTE = :poste, NUMTEL= :numt, EMAIL = :mail, PASSWORD = :pwd,GENRE = :gnr, DN= :dn WHERE NOM_EM = :nom");
    query.bindValue(":nom", nom);
    query.bindValue(":poste", poste);
    query.bindValue(":numt", numt);
    query.bindValue(":mail", mail);
    ...
    checkQuery.prepare("SELECT COUNT(*) FROM EMPLOYES WHERE NOM_EM = :nom AND POSTE = :poste AND NUMTEL = :numt AND EMAIL = :mail AND GENRE = :gnr AND DN = :dn");
...
{
    QString queryStr = "SELECT NOM_EM, POSTE, NUMTEL, EMAIL, PASSWORD, GENRE, DN FROM EMPLOYES";

    if (critere == "Nom") {
        queryStr += " ORDER BY NOM_EM ASC";
...
{
    QString queryStr = "SELECT NOM_EM, POSTE, NUMTEL, EMAIL, PASSWORD, GENRE, DN FROM EMPLOYES WHERE NOM_EM LIKE :criteres OR POSTE LIKE :criteres";
...
void MainWindow::onSearchClicked()
{
    ...
    if (searchText.isEmpty()) {
        ...
        query.prepare("SELECT NOM_EM, POSTE, NUMTEL, EMAIL, PASSWORD, GENRE, DN FROM EMPLOYES");
    } else {
        // Sinon, on fait une recherche partielle sur plusieurs champs (LIKE)
        query.prepare(R"(
    SELECT NOM_EM, POSTE, NUMTEL, EMAIL, PASSWORD, GENRE, DN
    FROM EMPLOYES
    WHERE UPPER(NOM_EM) LIKE UPPER(:search)
...
void MainWindow::searchEmployes(const QString &searchText)
{
    ...
    QString queryStr = QString("SELECT * FROM EMPLOYES WHERE NOM_EM LIKE '%%1%' OR POSTE LIKE '%%1%'")
                           .arg(searchText);
...
void MainWindow::refreshTable()
{
    ...
    QSqlQuery query("SELECT * FROM EMPLOYES");  // Récupérer tous les employés
...
void MainWindow::on_statbutton_clicked()
{
    ...
    for (const StatInfo &stat : stats)
    {
        ...
        QSqlQuery query;
        QString queryString = "SELECT " + stat.field + ", COUNT(*) AS count FROM EMPLOYES GROUP BY " + stat.field;
...
void MainWindow::onDashboardClicked() {
    ...
    QSqlQuery posteQuery("SELECT POSTE, COUNT(*) FROM EMPLOYES GROUP BY POSTE");
...
int MainWindow::calculateEmployeesInRange(int minAge, int maxAge) {
    QSqlQuery query;
    query.prepare("SELECT COUNT(*) FROM EMPLOYES "
...
QStringList MainWindow::getSecretQuestions() {
    ...
    QSqlQuery query("SELECT DISTINCT QUESTION FROM EMPLOYES WHERE QUESTION IS NOT NULL");
    while (query.next()) {
...
void MainWindow::onPasswordForgotClicked()
{
    ...
    connect(validateButton, &QPushButton::clicked, [=]() {
        ...
        if (verifySecretAnswer(nom, question, reponse)) {
            ...
            updateQuery.prepare("UPDATE EMPLOYES SET PASSWORD = :pwd WHERE NOM_EM = :nom");
