...
locataires::locataires(int id_loc,QString nom_loc,QString mail_loc,QDate date_inscrit,QDate date_fc,QString statut,QString mode_p,QString prix,QString num_loc) {
    this->id_loc = id_loc;
...
bool locataires::aj(){
    ...
    query.prepare("insert into locataires ( nom_loc, mail_loc, date_inscrit, date_fc, statut, mode_p, prix, num_loc) "
                  "values ( :nom_loc, :mail_loc, :date_inscrit, :date_fc, :statut, :mode_p, :prix, :num_loc)");
   ...
   // query.bindValue(":id_loc", res);  // Using the result directly
    ...
    query.bindValue(":num_loc", num_loc);
...
QSqlQueryModel* locataires::aff(){
    ...
    model->setHeaderData(8, Qt::Horizontal, QObject::tr("num_loc"));

    /*// Add extra columns for update and delete
    model->insertColumn(model->columnCount());
    model->insertColumn(model->columnCount());
    model->setHeaderData(9, Qt::Horizontal, QObject::tr("Update"));
    model->setHeaderData(10, Qt::Horizontal, QObject::tr("Delete"));*/
...
bool locataires::modif(int id_loc) {
    ...
    query.prepare("UPDATE locataires SET nom_loc = :nom_loc, mail_loc = :mail_loc, "
                  ...
                  "statut = :statut, mode_p = :mode_p, prix = :prix, num_loc = :num_loc WHERE id_loc = :id_loc");
...
QMap<QString, int> locataires::stat_payement() {
    QMap<QString, int> stats;
    QSqlQuery query;

    QStringList modes = {"Espece", "Cheque", "Virement"};
    for (const QString &mode_p : modes) {
        query.prepare("SELECT COUNT(*) FROM locataires WHERE mode_p = :mode_p");
        query.bindValue(":mode_p", mode_p);
        query.exec();
        if (query.next()) {
            stats[mode_p] = query.value(0).toInt();
        }
    }
    return stats;
}
