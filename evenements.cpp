...
bool evenement::modifier(QString old_nom) {
    if (old_nom.isEmpty()) {
        qDebug() << "No old name provided for modification.";
        throw std::runtime_error("Old event name is empty.");
    }
    ...
    qDebug() << "Attempting to update event:" << old_nom << "to new name:" << nom;
    ...
    qDebug() << "Update successful for event:" << old_nom;
    return true;
}
